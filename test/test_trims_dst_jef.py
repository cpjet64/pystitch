...
class TestTrims(CleanupTestCase):
    ...
    def test_dst_trims(self):
        ...
        assert loaded_pattern.count_stitch_commands(TRIM) != 0
    ...
    def test_dst_trims_success(self):
        ...
        loaded_pattern = read_dst(
            file0, {"trim_at": 50, "clipping": False}
        )  # Lines beyond 50 jumps get a trim.
        assert loaded_pattern is not None
        assert loaded_pattern.count_stitch_commands(TRIM) != 0
        self.addCleanup(os.remove, file0)
    ...
    def test_jef_trims_off(self):
        ...
        assert loaded_pattern.count_stitch_commands(JUMP) == 15
        assert loaded_pattern is not None
        assert loaded_pattern.count_stitch_commands(TRIM) == 0
        self.addCleanup(os.remove, file0)
    ...
    def test_jef_trims_commands(self):
        ...
        assert loaded_pattern.count_stitch_commands(TRIM) == 6
