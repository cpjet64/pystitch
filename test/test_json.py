...
class TestJson(CleanupTestCase):
    ...
    def test_metadata_write_read_json(self):
        ...
        assert w_pattern.extras["name"] == "My Embroidery."
        assert w_pattern.extras["value"] == 208
        assert len(pattern.threadlist) == len(w_pattern.threadlist)
        for q in range(0, len(pattern.threadlist)):
            assert pattern.threadlist[q] == w_pattern.threadlist[q]
    ...
    def test_write_read_json(self):
        ...
        for q in range(0, len(pattern.stitches)):
            assert pattern.stitches[q] == w_pattern.stitches[q]
        self.addCleanup(os.remove, file1)
